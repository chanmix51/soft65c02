...
pub fn asl(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
...
mod tests {
    ...
    #[test]
    fn test_asl() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "ASL", AddressingMode::ZeroPage([0x0a]), asl);
    ...
    #[test]
    fn test_asl_acc() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "ASL", AddressingMode::Accumulator, asl);
        let (mut memory, mut registers) = get_stuff(0x1000, vec![0xe8, 0x0a, 0x02]);
