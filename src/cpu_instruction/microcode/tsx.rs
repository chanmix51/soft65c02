...
pub fn tsx(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
    let resolution = cpu_instruction.addressing_mode
        .solve(registers.command_pointer, memory, registers)?;

    registers.register_x = registers.status_register;
...
mod tests {
    ...
    #[test]
    fn test_tsx() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "TSX", AddressingMode::Implied, tsx);
        let (mut memory, mut registers) = get_stuff(0x1000, vec![0x8a]);
        let log_line = cpu_instruction.execute(&mut memory, &mut registers).unwrap();
        assert_eq!("TSX".to_owned(), log_line.mnemonic);
        assert_eq!(0b00110000, registers.register_x);
    ...
    fn test_tsx_with_n_flag() {
        ...
        let log_line = cpu_instruction.execute(&mut memory, &mut registers).unwrap();
        assert_eq!(0b10110000, registers.register_x);
        assert!(!registers.z_flag_is_set());
        assert!(registers.n_flag_is_set());
        assert_eq!(0x1001, registers.command_pointer);
    }
    ...
    fn test_tsx_with_z_flag() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "tsx", AddressingMode::Implied, tsx);
        let (mut memory, mut registers) = get_stuff(0x1000, vec![0x8a, 0x0a, 0x02]);
        registers.status_register = 0x00;
        let log_line = cpu_instruction.execute(&mut memory, &mut registers).unwrap();
