...
pub fn ldy(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
    ...
    registers.register_y = memory.read(target_address, 1).unwrap()[0];
...
mod tests {
    ...
    #[test]
    fn test_ldy() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xa0, "LDY", AddressingMode::Immediate([0x0a]), ldy);
    ...
    fn test_ldy_negative() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "LDY", AddressingMode::Immediate([0x8a]), ldy);
    ...
    #[test]
    fn test_ldy_zero() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "LDY", AddressingMode::Immediate([0x00]), ldy);
