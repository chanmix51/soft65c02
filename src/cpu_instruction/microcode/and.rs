...
pub fn and(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
...
mod tests {
    ...
    #[test]
    fn test_and() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "AND", AddressingMode::Immediate([0x0a]), and);
    ...
    fn test_and_set_zero() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "AND", AddressingMode::Immediate([0x55]), and);
    ...
    fn test_and_set_negative() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "AND", AddressingMode::Immediate([0xaa]), and);
