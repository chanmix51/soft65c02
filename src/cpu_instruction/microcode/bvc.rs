...
pub fn bvc(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
    let resolution = cpu_instruction.addressing_mode
        .solve(registers.command_pointer, memory, registers)?;
    let target_address = resolution.target_address
        .expect("BVC must have operands, crashing the application");
...
mod tests {
    ...
    #[test]
    fn test_bvc() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "BVC", AddressingMode::Relative([0x0a]), bvc);
