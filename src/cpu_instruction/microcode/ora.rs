...
pub fn ora(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
...
mod tests {
    ...
    #[test]
    fn test_ora() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "ORA", AddressingMode::Immediate([0x0a]), ora);
    ...
    fn test_ora_set_zero() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "ORA", AddressingMode::Immediate([0x00]), ora);
