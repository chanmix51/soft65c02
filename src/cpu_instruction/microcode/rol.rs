...
pub fn rol(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
...
mod tests {
    ...
    #[test]
    fn test_rol() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "ROL", AddressingMode::ZeroPage([0x0a]), rol);
        ...
        assert_eq!(format!("#0x1000: (ca 0a)       ROL  $0a      (#0x000A)  0x50[S=nv-Bdizc]"), format!("{}", log_line));
    }

    #[test]
    fn test_rol_acc() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "ROL", AddressingMode::Accumulator, rol);
        let (mut memory, mut registers) = get_stuff(0x1000, vec![0xe8, 0x0a, 0x02]);
        ...
        assert_eq!(format!("#0x1000: (ca)          ROL  A                   [A=0x50][S=nv-Bdizc]"), format!("{}", log_line));
    }
