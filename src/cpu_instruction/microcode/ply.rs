...
pub fn ply(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
...
mod tests {
    ...
    #[test]
    fn test_ply() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0x08, "PLY", AddressingMode::Implied, ply);
        ...
        assert_eq!(format!("#0x1000: (08)          PLY                      [Y=0x10][SP=0xff][S=nv-Bdizc]"), format!("{}", log_line));
    }

    #[test]
    fn test_ply_zero() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "PLY", AddressingMode::Implied, ply);
