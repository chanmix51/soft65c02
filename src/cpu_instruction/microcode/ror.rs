...
pub fn ror(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
...
mod tests {
    ...
    #[test]
    fn test_ror() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "ROR", AddressingMode::ZeroPage([0x0a]), ror);
        ...
        assert_eq!(format!("#0x1000: (ca 0a)       ROR  $0a      (#0x000A)  0x14[S=nv-Bdizc]"), format!("{}", log_line));
    }

    #[test]
    fn test_ror_acc() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "ROR", AddressingMode::Accumulator, ror);
        let (mut memory, mut registers) = get_stuff(0x1000, vec![0xe8, 0x0a, 0x02]);
        ...
        assert_eq!(format!("#0x1000: (ca)          ROR  A                   [A=0x14][S=nv-Bdizc]"), format!("{}", log_line));
    }
