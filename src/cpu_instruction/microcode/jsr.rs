...
pub fn jsr(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
    ...
    stack_push(memory, registers, bytes[1]);
    stack_push(memory, registers, bytes[0]);
...
mod tests {
    ...
    #[test]
    fn test_jsr() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "JSR", AddressingMode::Absolute([0x0a, 0x20]), jsr);
        ...
        assert_eq!(vec![0x02, 0x10], memory.read(STACK_BASE_ADDR + 0xfe, 2).unwrap());
    }
}
