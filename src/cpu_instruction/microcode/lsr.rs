...
pub fn lsr(memory: &mut Memory, registers: &mut Registers, cpu_instruction: &CPUInstruction) -> Result<LogLine> {
    ...
    let (res, carry) = byte.overflowing_shr(1);
...
mod tests {
    ...
    #[test]
    fn test_lsr() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "LSR", AddressingMode::ZeroPage([0x0a]), lsr);
    ...
    #[test]
    fn test_lsr_acc() {
        let cpu_instruction = CPUInstruction::new(0x1000, 0xca, "LSR", AddressingMode::Accumulator, lsr);
        let (mut memory, mut registers) = get_stuff(0x1000, vec![0xe8, 0x0a, 0x02]);
